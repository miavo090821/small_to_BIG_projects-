...
class NetworkManager:
    # players from afar can join in by just inputting the IP address
    def get_ip_address():
        try:
    ...
    def draw_hangman(self, stage, player_index):
        ...
        for i in range(stage + 1):
            ...
            if len(graphic) == 2:
                ...
                circle_center = (head_x, head_y, head_radius)
                pygame.draw.circle(window, WHITE, (head_x, head_y), head_radius)

    def run_hangman_game(self, player_name, player_index):
        # Run a game of hangman
        congrats_sound, correct_sound, incorrect_sound = self.sound_manager.congrats_sound, \
        ...
        while True:
            ...
            print()
# now combine the sound into game: 
            ...
            if len(guessed_letters) >= max_guesses:
                ...
                break
        # nicely mentioning the score 
        # so that it will be stored into a file for high scores data 
            print("Score:", score)
            if score == 0:
                print("You lost")
    ...
    # starting game loop
        while True:
            choice = input(
                "Do you want to register (R), login (L), view high scores (H), or search for IP addresses (I)? ").lower()
